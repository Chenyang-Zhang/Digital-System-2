...
int main(){
	...
	printf("Calculation is done. The correct rate is %f", (float)correct_rate/MAX_CALCULATION);
}
...
float add_float(Float m, Float n){
	...
	if(diff > 0){       //第一种情况
		...
		while( (mantissa_r | 0x7fffff) != 0xffffff){
				...
				result.exponent --;
				loop ++;
				if(loop > MAX_LOOPTIME){
						printf("Error! mantissa = %u\n", mantissa_r);
						break;
				}
		}
